...
8. **Weights are composed, not yet measured** (`weights.rs`). The benchmarks exist (`benchmarking.rs`: one per call, `finalize_retirement` linear in the queue length `n ≤ MaxUnlockingChunks`, everything else at `MaxTargets`), run against the mock under `--features runtime-benchmarks` and are registered in the testnet runtime's `define_benchmarks!` behind `Config::BenchmarkHelper` (validators, the cooperator gate, the era clock — `mock.rs` and the runtime each implement it). The numbers must come from the hardware in `pallets/BENCHMARKING.md`; until then the composed figures stand.
