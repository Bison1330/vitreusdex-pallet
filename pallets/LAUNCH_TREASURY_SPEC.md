...
12. **Two branches carry this pallet, at different bases; which is authoritative for what.** `design/launch-treasury` (on `pr/dex-launchpad` `423740e`, `spec_version` 213) is *what gets submitted*: the pallet, D9 as the DEX's v2 and L1 as the launchpad's v1 without migrations, the benchmarks and measured weights, this spec. `feature/solver-marketplace` (the fork, `spec_version` 221) is *what runs* on the dev chain: the same commits cherry-picked with `-x` (so `git log --grep='cherry picked from'` on the fork lists exactly what has crossed), plus what only a running chain needs and the submission must not carry — the DEX v3 and launchpad v1 migrations (`migrations::v3`, `migrations::v1`, since the fork's chain has pools and launches in the old shapes), the fork's own runtime layout (`mod launch_treasury` beside its top-level DEX and launchpad wiring), the spec bump, and the pre-flight record in `dev-ops/preflight/`. Rules: a pallet, runtime-adapter or spec change is made on `design/launch-treasury` first and cherry-picked to the fork, never the reverse; the fork's migration modules, `Unreleased` list and `spec_version` are never ported back; when a pick conflicts, the fork resolves it in the pick itself (the `-x` line still names the source) or in a following commit that names the PR commit it completes. A bug fixed on one branch and not the other is the failure this item exists to prevent: check both `git log`s before calling it fixed.
